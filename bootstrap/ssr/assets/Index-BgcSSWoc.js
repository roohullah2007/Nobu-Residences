...
function Index({ auth }) {
  ...
  const deleteMessage = (website) => {
    const domainNote = website.domain ? ` The custom hostname "${website.domain}" will also be removed from Cloudflare (its edge certificate is dedicated, so no other domain is affected).` : "";
