...
const PersistedRow = ({ title, status, timestamp, timestampLabel, pendingMessage }) => {
  ...
  return /* @__PURE__ */ jsxs("div", { className: "border border-gray-200 rounded-lg p-4", children: [
    ...
    status === "waiting_dns" && /* @__PURE__ */ jsx("div", { className: "text-xs text-gray-500 mt-1", children: "The domain does not point to the server yet. DNS is checked every 5 minutes — the alias and SSL certificate will be provisioned automatically as soon as the A record points here. Nothing to do besides updating DNS." })
  ] });
};
const Status = ({ status }) => {
  if (status === true) {
