...
export default function GoogleLoginButton({ text = "Sign in with Google", className = "" }) {
    // Forward the current page as `redirect_to` so the OAuth callback can
    // bring the user back to where they triggered the login modal.
    const href = (() => {
        if (typeof window === 'undefined') return '/auth/google';
        const here = `${window.location.pathname}${window.location.search}${window.location.hash}`;
        return `/auth/google?redirect_to=${encodeURIComponent(here)}`;
    })();
